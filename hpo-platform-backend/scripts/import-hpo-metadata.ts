...
async function downloadHPOBO(): Promise<string> {
  ...
  try {
    // URL atualizada (redirect da antiga)
    const response = await axios.get('https://github.com/obophenotype/human-phenotype-ontology/releases/latest/download/hp.obo', {
      responseType: 'stream',
      timeout: 120000, // 120s timeout (arquivo grande)
      maxRedirects: 5,
    });
...
/**
 * Importa TODOS os termos do OBO para o database
 */
async function importAllFromOBO(terms: Map<string, OBOTerm>) {
  console.log('📥 Importando todos os termos do hp.obo...\n');
  ...
  // FASE 1: Criar todos os termos sem parentId (para evitar dependências)
  console.log('   FASE 1/2: Criando termos...');
  ...
  for (let i = 0; i < termsArray.length; i += batchSize) {
    ...
    for (const oboTerm of batch) {
      ...
      try {
        ...
        const category = findRootCategory(oboTerm.id, terms);
        
        // Criar termo SEM parentId primeiro
        await prisma.hpoTerm.upsert({
          where: { hpoId: oboTerm.id },
          create: {
            hpoId: oboTerm.id,
            labelEn: oboTerm.name,
            definitionEn: oboTerm.definition || null,
            synonymsEn: oboTerm.synonyms,
            category: category || null,
            parentId: null, // Fase 1: não define parent ainda
        ...
        // Log progresso a cada 1000 termos
        if (created % 1000 === 0) {
          console.log(`   ⏳ ${created}/${termsArray.length} termos criados...`);
        }
        
      } catch (error: any) {
        console.error(`   ❌ Erro em ${oboTerm.id}:`, error.message);
        errors++;
      }
    }
  }
  
  // FASE 2: Atualizar parentId agora que todos os termos existem
  console.log('\n   FASE 2/2: Atualizando hierarquia (parentId)...');
  updated = 0;
  
  for (const oboTerm of termsArray) {
    if (oboTerm.obsolete || oboTerm.parents.length === 0) continue;
    
    try {
      const parentHpoId = oboTerm.parents[0];
      const [childInDb, parentInDb] = await Promise.all([
        prisma.hpoTerm.findUnique({
          where: { hpoId: oboTerm.id },
          select: { id: true }
        }),
        prisma.hpoTerm.findUnique({
      ...
      if (childInDb && parentInDb) {
        ...
        if (updated % 1000 === 0) {
          console.log(`   ⏳ ${updated} relações parent-child criadas...`);
        }
      }
    } catch (error: any) {
      // Ignorar erros silenciosamente (muitos termos não têm pais)
    }
  }
  
  console.log('\n═══════════════════════════════════════');
  console.log(`✅ Importado:   ${created} termos`);
  console.log(`🔗 Hierarquia:  ${updated} relações parent-child`);
  console.log(`⏭️  Obsoleto:   ${obsolete} termos (pulados)`);
  ...
  return { created, obsolete, errors };
}
...
async function enrichDatabase(terms: Map<string, OBOTerm>) {
  ...
  console.log(`📦 ${terms.size} termos no hp.obo`);
  
  // Se database tem menos de 80% dos termos do OBO, importar TUDO
  const percentInDb = (dbTerms.length / terms.size) * 100;
  console.log(`📊 Database tem ${percentInDb.toFixed(1)}% dos termos do OBO`);
  
  if (percentInDb < 80) {
    console.log('\n⚠️  Database incompleto. Importando TODOS os termos do hp.obo...\n');
    return await importAllFromOBO(terms);
  }
  
  console.log('\n✅ Database completo! Apenas enriquecendo metadados existentes...\n');
  
  let updated = 0;
  let notFound = 0;
  let errors = 0;
  
  // FASE 1: Atualizar category (rápido, sem lookup)
  console.log('   FASE 1/2: Atualizando categories...');
  ...
  for (const dbTerm of dbTerms) {
    ...
    try {
      ...
      // Log progresso a cada 500 termos
      if (updated % 500 === 0) {
        console.log(`   ⏳ ${updated}/${dbTerms.length} categories...`);
      }
  ...
  // FASE 2: Criar mapa de hpoId → database id (para lookup rápido)
  const hpoIdToDbId = new Map<string, string>();
  const allTerms = await prisma.hpoTerm.findMany({
    select: { id: true, hpoId: true }
  });
  ...
  console.log(`   📋 Mapa criado com ${hpoIdToDbId.size} termos\n`);
  
  // FASE 2: Atualizar parentId usando o mapa (rápido)
  let hierarchyUpdated = 0;
  
  for (const dbTerm of dbTerms) {
    const oboTerm = terms.get(dbTerm.hpoId);
    
    if (!oboTerm || oboTerm.parents.length === 0) {
      continue;
    }
    ...
    try {
      ...
      if (parentDbId) {
        ...
        if (hierarchyUpdated % 500 === 0) {
          console.log(`   ⏳ ${hierarchyUpdated} relações parent-child...`);
        }
      }
  ...
  console.log('\n═══════════════════════════════════════');
  console.log(`✅ Categories:  ${updated} termos`);
  console.log(`🔗 Hierarquia:  ${hierarchyUpdated} relações`);
